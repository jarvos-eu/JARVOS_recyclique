"""Tests API CRUD presets + GET /active, validation category_id — Story 2.4."""
...
def _create_preset(
    ...
    resp = client.post("/v1/presets", json=body)
    assert resp.status_code == 201
    return resp.json()["id"]
...
def test_list_presets_returns_created(client: TestClient):
    """GET /v1/presets retourne les presets crees."""
    _create_preset(client, name="P A", button_type="Don")
    _create_preset(client, name="P B", button_type="Recyclage")
    resp = client.get("/v1/presets")
    assert resp.status_code == 200
...
def test_list_presets_filter_category_id(client: TestClient):
    """GET /v1/presets?category_id=... filtre par categorie."""
    cat_id = _create_category(client, name="Cat Presets")
    _create_preset(client, name="P Linked", category_id=cat_id)
    _create_preset(client, name="P No Cat")  # no category
    resp = client.get("/v1/presets", params={"category_id": cat_id})
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
...
def test_get_presets_active_only_active_sorted(client: TestClient):
    """GET /v1/presets/active retourne seulement is_active=true, tri par sort_order."""
    _create_preset(client, name="Inactive", is_active=False, sort_order=0)
    _create_preset(client, name="Active 2", is_active=True, sort_order=20)
    _create_preset(client, name="Active 1", is_active=True, sort_order=10)
    resp = client.get("/v1/presets/active")
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 2
...
def test_get_presets_active_route_before_id(client: TestClient):
...
def test_get_preset_not_found(client: TestClient):
    """GET /v1/presets/{id} inexistant retourne 404 et detail."""
    resp = client.get("/v1/presets/00000000-0000-0000-0000-000000000000")
    assert resp.status_code == 404
    assert "detail" in resp.json()
    assert resp.json()["detail"] == "Preset not found"


def test_get_preset_after_create(client: TestClient):
...
def test_create_preset_201(client: TestClient):
...
def test_create_preset_with_category_id(client: TestClient):
    """POST /v1/presets avec category_id valide lie la categorie."""
    cat_id = _create_category(client, name="Cat For Preset")
    resp = client.post(
        "/v1/presets",
        json={
...
def test_create_preset_category_not_found_400(client: TestClient):
...
def test_patch_preset(client: TestClient):
    """PATCH /v1/presets/{id} met a jour partiellement."""
    preset_id = _create_preset(client, name="Original", preset_price=0)
    resp = client.patch(
...
def test_patch_preset_category_id_valid(client: TestClient):
    """PATCH /v1/presets/{id} avec category_id existant met a jour."""
    cat_id = _create_category(client, name="Cat Patch")
    preset_id = _create_preset(client, name="P", category_id=None)
    resp = client.patch(
        f"/v1/presets/{preset_id}",
...
def test_patch_preset_category_id_unset_null(client: TestClient):
    """PATCH /v1/presets/{id} avec category_id: null decroche la categorie."""
    cat_id = _create_category(client, name="Cat To Unset")
    preset_id = _create_preset(client, name="P With Cat", category_id=cat_id)
    resp = client.patch(
        f"/v1/presets/{preset_id}",
...
def test_patch_preset_category_not_found_400(client: TestClient):
    """PATCH /v1/presets/{id} avec category_id inexistant retourne 400."""
    preset_id = _create_preset(client, name="P")
    resp = client.patch(
        f"/v1/presets/{preset_id}",
...
def test_patch_preset_not_found(client: TestClient):
...
def test_delete_preset_204(client: TestClient):
    """DELETE /v1/presets/{id} suppression definitive (204), preset absent de la liste."""
    preset_id = _create_preset(client, name="To delete")
    resp = client.delete(f"/v1/presets/{preset_id}")
    assert resp.status_code == 204
    list_resp = client.get("/v1/presets")
    names = [p["name"] for p in list_resp.json()]
    assert "To delete" not in names
    get_resp = client.get(f"/v1/presets/{preset_id}")
    assert get_resp.status_code == 404


def test_delete_preset_not_found(client: TestClient):
    """DELETE /v1/presets/{id} inexistant retourne 404."""
    resp = client.delete("/v1/presets/00000000-0000-0000-0000-000000000000")
    assert resp.status_code == 404
