"""Tests API CRUD + GET /status /v1/cash-registers — Story 2.2."""

import pytest
from fastapi.testclient import TestClient


def _create_site(client: TestClient, name: str = "Site Test", is_active: bool = True):
    resp = client.post("/v1/sites", json={"name": name, "is_active": is_active})
    assert resp.status_code == 201
    return resp.json()["id"]


def test_list_cash_registers_empty(client: TestClient):
    """GET /v1/cash-registers retourne 200 et une liste vide quand aucun poste."""
    resp = client.get("/v1/cash-registers")
    assert resp.status_code == 200
    assert resp.json() == []


def test_list_cash_registers_filter_site_id(client: TestClient):
    """GET /v1/cash-registers?site_id=... retourne 200."""
    site_id = _create_site(client)
    resp = client.get("/v1/cash-registers", params={"site_id": site_id})
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)


def test_list_cash_registers_filter_is_active(client: TestClient):
    """GET /v1/cash-registers?is_active=true retourne 200."""
    resp = client.get("/v1/cash-registers", params={"is_active": True})
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)


def test_get_cash_registers_status_empty(client: TestClient):
    """GET /v1/cash-registers/status retourne 200 et liste vide quand aucun poste."""
    resp = client.get("/v1/cash-registers/status")
    assert resp.status_code == 200
    assert resp.json() == []


def test_get_cash_registers_status_with_registers(client: TestClient):
...
def test_get_cash_registers_status_filter_site_id(client: TestClient):
    """GET /v1/cash-registers/status?site_id=... filtre par site."""
    site_id = _create_site(client)
    resp = client.get("/v1/cash-registers/status", params={"site_id": site_id})
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)


def test_get_cash_register_not_found(client: TestClient):
...
def test_create_cash_register_201(client: TestClient):
...
def test_create_cash_register_site_not_found_404(client: TestClient):
...
def test_get_cash_register_after_create(client: TestClient):
    ...
    resp = client.get(f"/v1/cash-registers/{register_id}")
    assert resp.status_code == 200
    assert resp.json()["name"] == "Caisse B"
    assert resp.json()["is_active"] is False


def test_list_cash_registers_returns_created(client: TestClient):
...
def test_patch_cash_register(client: TestClient):
...
def test_patch_cash_register_empty_body_does_not_update_updated_at(client: TestClient):
    ...
    resp = client.patch(f"/v1/cash-registers/{register_id}", json={})
    assert resp.status_code == 200
    assert resp.json()["name"] == "No change"
    assert resp.json()["updated_at"] == updated_before


def test_patch_cash_register_not_found(client: TestClient):
...
def test_delete_cash_register_204(client: TestClient):
    ...
    register_id = create.json()["id"]
    resp = client.delete(f"/v1/cash-registers/{register_id}")
    assert resp.status_code == 204
    get_resp = client.get(f"/v1/cash-registers/{register_id}")
    assert get_resp.status_code == 404


def test_delete_cash_register_not_found(client: TestClient):
