...
- **Point d’entrée HTTP :** `src/www/_route.php` → `Router::route()` (`Paheko\Web\Router`).
- **Racine projet :** `src/include/init.php` charge la config, l’autoload (dont namespace `Paheko\Plugin\` pour les plugins), constantes (ROOT, PLUGINS_ROOT, etc.).
