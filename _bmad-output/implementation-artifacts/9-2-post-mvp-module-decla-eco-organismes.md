...
- api/core/modules/loader.py (modifié — registre decla DeclaModule, get_enabled_modules pour main)

- api/core/modules/decla_module.py (nouveau)

- api/main.py (modifié — inclusion conditionnelle route export décla via get_enabled_modules)

- api/routers/declarative.py (modifié — utilise get_aggregates, constantes partagées depuis service)

- api/routers/declarative_export.py (nouveau)

- api/services/declarative_service.py (modifié — get_aggregates, VALID_FLOW_TYPES, YEAR_MIN/MAX exportés)
...
- **Résultat :** Approuvé après corrections appliquées lors du review.

- **Vérifications :** AC1–AC3 validés (module activable via modules.toml, export CSV/JSON basé sur agrégats, paramètre eco_organism réservé). Toutes les tâches marquées [x] sont implémentées. Tests 16/16 passent (dont export 200 CSV/JSON, 422 format invalide, 403 sans admin).

- **Corrections appliquées pendant le review :** (1) Duplication des constantes VALID_FLOW_TYPES, YEAR_MIN, YEAR_MAX supprimée — centralisées dans `api/services/declarative_service.py`, importées par les deux routers. (2) Double lecture du TOML dans `main.py` supprimée — `loader` expose `get_enabled_modules()` et `main` l'utilise pour inclure la route export décla.

- **Remarques mineures (non bloquantes) :** File List ne mentionne pas `api/models/__init__.py` ni `doc/index.md` si modifiés dans le même lot (traçabilité). Aucun test explicite pour « module decla désactivé → 404 sur /export » (acceptable post-MVP).
...
| 2026-02-27 | AI QA  | Code review adversarial : 2 corrections MEDIUM appliquées (constantes partagées, get_enabled_modules). Status → done. |
