...
- [x] Task 5 — Détail session admin (AC: 1, 2)
  - [x] Aligner AdminCashSessionDetailPage : détail session (ouverture, clôture, fond, montants, écart), liste ventes, lignes, paiements, poids ; actions édition item (destination, prix), édition poids.
  - [x] Vérifier GET /v1/cash-sessions/{id} ; PUT /v1/sales/{sale_id} ; PATCH items/weight et items ; appliquer checklist import.
...
**Console** : Vérification manuelle recommandée sur les 5 URLs (dashboard, session/open, sale, close, admin/cash-sessions/:id) ; à documenter par le relecteur ou en recette (« Vérification console OK » si aucune erreur rouge).
