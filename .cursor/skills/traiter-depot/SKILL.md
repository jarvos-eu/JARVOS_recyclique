...
- Lire le skill **idees-kanban** (`~/.cursor/skills/idees-kanban/SKILL.md`) et, pour **chaque** idée extraite, appliquer l'opération **`capture`** (une fiche `IDEA-*.md` dans `docs/ideas/kanban/`).
- Ensuite, ventiler le **fichier source** (la liste) : il ne va **pas** dans `docs/ideas/kanban/` (réservé aux fiches une-idée-un-fichier). Le placer vers vision-projet ou artefacts (ex. `YYYY-MM-DD_archive_liste-idees-importees.md`).
---
...
Ne pas mettre à jour `docs/ideas/kanban/INDEX.md` dans ce skill — c'est géré par le skill idees-kanban.
